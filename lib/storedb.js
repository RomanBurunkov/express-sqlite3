...
module.exports = class StoreDb {
  ...
  create(cb) {
    this.client.run(
      this.concurrentDb ? `PRAGMA journal_mode = wal;${this.schema()}` : this.schema(),
      (err) => processCallback(err, null, cb),
    );
  }
  ...
  cleanup(cb) {
    this.client.run(
      `DELETE FROM ${this.table} WHERE ? > expired`,
      Date.now(),
  ...
  count(cb) {
    ...
    this.client.get(
      ...
      (e, row) => processCallback(e, row, cb, (r) => r.count),
    );
  }
};
